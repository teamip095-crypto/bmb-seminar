  ...
  public static async generateQuizForAttempt(params: {
    ...
    if (ai && process.env.GEMINI_API_KEY) {
      try {
        ...
        const prompt = `
Generate exactly 5 easy multiple-choice questions in professional, clean, easy-to-understand HINDI for the BMB Educom AI Seminar Preliminary Quiz.
...
2. Language: 100% Hindi (Devanagari script, standard terminology like AI, Machine Learning, ChatGPT can be in Hindi).
3. Topic coverage:
   - Question 1: AI Basics & Fundamentals (Easy)
   - Question 2: Generative AI & AI Awareness (Easy)
   - Question 3: Everyday AI Tools & Real-world Awareness (Easy)
   - Question 4: BMB Educom Learning & Practical Project Mastery (Easy, grounded in provided knowledge base)
   - Question 5: BMB Educom Seminar & Career Opportunities (Easy, grounded in provided knowledge base)
...
`;
          ...
          config: {
            ...
            responseSchema: {
              ...
              properties: {
                questions: {
                  type: Type.ARRAY,
                  description: "Array of exactly 5 Hindi quiz questions",
                  items: {
                    type: Type.OBJECT,
                    properties: {
  ...
  public static selectBalancedQuestionsFromBank(seed: string): GeneratedQuizQuestion[] {
    ...
    const q3 = pickUnique(businessCareerQuestions.length > 0 ? businessCareerQuestions : aiCoreQuestions, 37);
    const q4 = pickUnique(bmbQuestions, 73);
    const q5 = pickUnique(bmbQuestions, 109);
