...
export class FridayEngine {
  ...
  /**
   * Retrieves or creates the current seminar event based 100% on Admin Settings
   * No hardcoded Friday logic: whatever date, day, time, venue admin sets is authoritative.
   */
  public static async getOrCreateCurrentSeminar(simulatedDate?: Date): Promise<FridaySeminarStatus> {
    ...
    const formattedHi = settings.seminar_date_hi || "आगामी सेमिनार";
    const formattedEn = settings.seminar_date_en || "Upcoming Seminar";
    const seminarTime = settings.seminar_time || "11:00 AM – 4:00 PM IST";
    const venueLocation = settings.venue_location || "BMB Educom टेक हब (जयपुर / ऑनलाइन एक्सेस)";
    const reportingTime = settings.reporting_time || "10:45 AM";
    const isRegistrationOpen = settings.is_registration_open !== false;

    // Use current date or date string from admin
    const targetDateStr = nowIST.dateString;

    // Find active event or create one matching current admin settings
    let event = db.getAllSeminarEvents().find(e => e.status === "active");
    ...
    if (!event) {
      event = db.createSeminarEvent({
        ...
        title: `BMB Educom AI Seminar — ${formattedEn}`,
        description: `Official AI Learning, Quiz & Admission Event (${formattedHi})`,
        status: "active",
        registration_open: isRegistrationOpen
      });
    } else {
      // Keep existing active event synchronized with current Admin settings
      event.title = `BMB Educom AI Seminar — ${formattedEn}`;
      event.description = `Official AI Learning, Quiz & Admission Event (${formattedHi})`;
      event.registration_open = isRegistrationOpen;
      event.start_time = seminarTime;
    }
    ...
    return {
      ...
      isToday: true,
      isActiveNow: true,
      isRegistrationOpen: isRegistrationOpen && event.status !== "cancelled",
      formattedDateHi: formattedHi,
