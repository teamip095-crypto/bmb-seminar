...
export interface QuizResult {
  ...
  seminar_event_id: string;
  score: number; // 0 to 5
  total_questions: number; // 5
  duration_seconds: number;
  ranking_position?: number;
  result_status: "passed" | "participated" | "time_out";
  is_top10_winner?: boolean;
  free_pass_granted?: boolean;
  created_at: string;
}
...
export interface ScholarshipAttempt {
  ...
  started_at: string;
  expires_at: string;
  duration_seconds_limit: number; // 300 seconds (5 minutes)
  status: "in_progress" | "submitted" | "expired" | "timeout_auto_submitted";
  created_at: string;
  questions?: {
    id: string;
...
export interface ScholarshipSubmission {
  ...
  rank?: number;
  prize_text?: string;
  prize_type?: "cash_3000" | "cash_2000" | "cash_1500" | "gift_scholarship_1000" | "scholarship_500" | "cash_1000" | "cash_500" | "cash_200" | "gift" | "participated";
  cash_prize?: number;
...
export interface WhatsAppMessage {
  ...
  participant_id: string;
  phone_number: string;
  message_type: "registration_confirmation" | "quiz_link" | "leaderboard_update" | "admission_followup" | "admin_otp_recovery" | "admin_password_updated" | "admission_pass" | "seminar_reminder";
...
export const AdminSetupInputSchema = z.object({
  name: z.string().min(2, "कृपया एडमिन का नाम दर्ज करें (कम से कम 2 अक्षर)"),
  whatsapp_number: z.string().regex(/^[6-9]\d{9}$/, "कृपया 10 अंकों का मान्य भारतीय WhatsApp नंबर दर्ज करें"),
  email: z.string().email("कृपया मान्य ईमेल पता दर्ज करें").optional().or(z.literal("")),
...
export const SeminarSettingsUpdateSchema = z.object({
  ...
  is_stage2_active: z.boolean().optional(),
  stage2_activated_at: z.string().optional(),
  cash_prize_1st: z.coerce.number().min(0).optional(),
  cash_prize_2nd: z.coerce.number().min(0).optional(),
  cash_prize_3rd: z.coerce.number().min(0).optional(),
  cash_prize_consolation: z.string().optional(),
  scholarship_1st: z.coerce.number().min(0).optional(),
  scholarship_2nd: z.coerce.number().min(0).optional(),
  scholarship_3rd: z.coerce.number().min(0).optional(),
  scholarship_4_to_20: z.coerce.number().min(0).optional(),
  scholarship_participation: z.coerce.number().min(0).optional(),
  upi_id: z.string().optional(),
  upi_phone: z.string().optional(),
  pass_price_discounted: z.coerce.number().min(0).optional(),
  pass_price_original: z.coerce.number().min(0).optional(),
});
